...
##
## Code for the GLM shown on Supplementary Figure 3 and Tables 5a, 5b and 5c
## 1. Packages
...
coll_add <- function(x) {paste("n.sp.det~",paste(x, collapse="+"), sep="")}
coll_int <- function(x) {paste("n.sp.det~",paste(x, collapse="*"), sep="")}
...
## 4. Preparation of the files for the analysis
# Overwrite original species list to removed Felis sp, Mus musculus and Rangifer tarandus
...
richness.edna <- edna.detect %>%
  ...
  group_by(id, rain, aws, vol, year, time) %>% 
...
det.dna <- n.sp.det <- df$n.taxa # nb of taxa detected per sample
...
best <- glm(det.dna~aws+rain+vol+ year, family=poisson)
summary(best)


###########################################################################
#### Analysis on year 2018 only
# sum the number of taxa detected per sample
richness.edna <- edna.detect %>%
  filter(taxa %in% list.sp.all#,
         #year==2018
  ) %>%
  group_by(id, rain, aws, vol, year) %>% 
  summarise(n.taxa = sum(det.dna))
...
# Beta parameter estimates from the best model
best <- glm(n.sp.det~aws+rain+vol, family=poisson)
summary(best)
