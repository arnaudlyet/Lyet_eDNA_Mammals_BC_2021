#################################################################################################################
######  Modelling stream eDNA species detection in relation to environmental ####################################
######  covariates, sampling covariates, species ecological traits (diet and average weight)  ###################
## 
## Arnaud Lyet      
## Started October 28th 2020
## Last update May 18th 2020
##
## Code for the GLM shown on Figure 4 and Supplementary Tables 3a, 3b and 3c
## 1. Packages
...
## File with the list of species, groups and families
source("SpeciesLists.R")
...
sp.fact <- read_csv("sp_cofactor.csv") # describes diet and average weight of each taxa
...
# Overwrite the former species list to removed domestic species, grouped canis, grouped small ground rodents
list.sp.ct <- c("Ursus americanus", "Ursus arctos",   
                "Vulpes vulpes", # Canis spp removed because diet mixed between omnivorous (dog, coyote) and carnivorous (wolf) 
...
df <- left_join(df, sp.fact, by="taxa")

df <- df %>% mutate(diet = factor(diet, levels=c("animal","mixed","plant")))
...
params <- c("det.ws","rain","aws","vol","diet","wgt","year") 
...
best <- glm(det.dna~det.ws+rain+aws+vol+diet+wgt, family=binomial)
summary(best)
